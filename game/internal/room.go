...
func (r *Room)startGame()  {
	...
	//	发牌
		...
		for _,v := range r.RoomUserId{
			...
			user.Cardings = append([]int{},r.CardsBase[(0 + i * 13) : (13 + i * 13)]...)
...
//广播
func (r *Room)boarding(messge string)  {
	length := len(r.RoomUserId)
	for i :=0; i< length; i++{
		player := accIDUsers[r.RoomUserId[i]]
		player.WriteMsg(&msg.Boarding{Message:messge})
	}
...
func (r *Room)playCard()  {
	...
	//王牌为最后一张牌 即倒是第一张
	cardLength := len(r.CardsBase)
	r.WValueCard = r.CardsBase[cardLength-1]
	r.getWIndex()
	fmt.Println("鬼牌:",r.WValueCard,r.WIndexCard)
	for i := 4 * 13; i<cardLength -2 ; i++{
		...
		select {//自己牌的检测 是否可以胡 或者杠
			case flag  := <- player.MyTurn:
				if flag == 100 {//自个胡牌
					//player.WriteMsg(&msg.MimeHu{1,r.OutCard})
					str := "玩家"+ player.userData.AccID +"自摸"
					r.boarding(str)
					break
				}else if flag == 111{//自个杠 为暗杠
					str := "玩家"+ player.userData.AccID +"暗杠"
					r.boarding(str)
					continue
				}else if flag == 112{//明杠
					str := "玩家"+ player.userData.AccID +"明杠"
					r.boarding(str)
					continue
				}else {//没有胡与杠 出牌 放杠不在这里处理
					r.OutCard = flag
					str := "玩家"+ player.userData.AccID +"出牌:"+strconv.Itoa(r.OutCard)
					r.boarding(str)
				}
			case <- time.After( 60 * time.Second)://超时 玩家没有动静 则出最后一张牌
				...
				str := "玩家ID为:"+ player.userData.AccID + "超时打出的牌为:" + strconv.Itoa(r.OutCard)
				player.rspAllCards()
				r.boarding(str)
		}
			...
			if j == startPosition{
				...
				go func(player UserLine, r *Room) {
					...
					case flag := <- player.SumChan:
						...
						//fmt.Println("strFlagArray",strFlagArray)
						switch strFlagArray[0] {
						case "peng":
							fmt.Println("玩家ID：",player.userData.AccID," 叫碰")
							r.userWant["peng"] = make(map[int]string)
						...
						case "gang":
							...
							r.userWant["gang"] = make(map[int]string)
		...
		if len(r.userWant) == 0{
			...
			if len(r.userWant["fire"]) >0{
				...
				//	r.Playing
					var str string
					for i := 1; i <= 3; i++{
						if v, ok :=(r.userWant["fire"])[i]; ok{
							str = v
							break
						}
					}
					s := "玩家"+ str +"吃胡:"+strconv.Itoa(r.OutCard)
					r.boarding(s)
					r.initRoom()
				}else{//多个玩家吃胡
					for i:=1 ; i <= 3; i++{//
						if id, ok :=(r.userWant["fire"])[(startPosition+i)%4]; ok{//是下家胡?
							...
							str := "玩家iD"+id+"吃胡了"+strconv.Itoa(r.OutCard)
							r.boarding(str)
							r.initRoom()
							break
						//	初始化 游戏结束
			...
			//有一家杠 放杠处理
				user := accIDUsers[r.gangUserID]
				r.Playing = user.userData.AccID
				//fmt.Println("有一个玩家杠---------:",user.userData.AccID)
			...
			//	碰返回 去掉碰的 返回手牌
				...
				if v, ok := um[user.RoomPosition]; ok{
					...
					user.pengOK(index)
					str := "玩家:"+user.userData.AccID+"碰!"
					r.boarding(str)
					user.rspAllCards()
				}
				fmt.Println("等待玩家",user.userData.AccID,"出牌....")
				select {
				...
				str := "玩家"+user.userData.AccID+"出牌:"+strconv.Itoa(r.OutCard)
				r.boarding(str)
				startPosition = user.RoomPosition
				goto Again
